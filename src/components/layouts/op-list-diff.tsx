		...
		<div className="mb-4 space-y-2 mt-2">
			{removedOp.map((address, index) => (
				<div className="text-xs" key={address}>
					<a className="text-red-700 line-through" href={`https://${appConfig.TESTNET ? 'testnet' : ''}explorer.obyte.org/address/${address}`} target="_blank" rel="noopener"><span className="address">{address}</span> {address in appConfig.PROVIDER_DICTIONARY ? <span> ({appConfig.PROVIDER_DICTIONARY[address].displayName})</span> : null}</a>
					{" -> "}
					<a className="text-green-700" href={`https://${appConfig.TESTNET ? 'testnet' : ''}explorer.obyte.org/address/${addedOp[index]}`} target="_blank" rel="noopener"><span className="address">{addedOp[index]}</span> {addedOp[index] in appConfig.PROVIDER_DICTIONARY ? <span> ({appConfig.PROVIDER_DICTIONARY[addedOp[index]].displayName})</span> : null}</a>
				</div>
			))}
		</div>
	</>
